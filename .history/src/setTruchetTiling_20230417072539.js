        ...
        for (let x = 0; x < truchetRows; x++) {
            ...
            if (tileAbove != null) {
                ...
                if (tileMatrix[y][x].topRightClass == 'truchetTopRight' && tileAbove.bottomRightClass == 'truchetBottomRight') {
                    ...
                    if (tileMatrix[y][x].backPiece.colorGroup.includes() == false) {
                        tileMatrix[y][x].backPiece.colorGroup.push(tileAbove.backPiece.colorGroup[tileAbove.backPiece.colorGroup.length - 1]);
                    }
    ...
    for (let y = 0; y < truchetRows; y++) {
        for (let x = 0; x < truchetRows; x++) {
            tileMatrix[y][x].backPiece.colorGroup.forEach((group) => {
                if (topLevelContiguousGroups.includes(group.backPiece.colorGroup) == false) {
                    topLevelContiguousGroups.push(group.backPiece.colorGroup);
                }
                if (topLevelContiguousGroups.includes(group.cornerPieceBottom.colorGroup) == false) {
                    topLevelContiguousGroups.push(group.cornerPieceBottom.colorGroup);
                }
                if (topLevelContiguousGroups.includes(group.corberPieceTop.colorGroup) == false) {
                    topLevelContiguousGroups.push(group.corberPieceTop.colorGroup);
                }
            })
