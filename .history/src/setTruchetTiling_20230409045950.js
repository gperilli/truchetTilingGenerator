...
function setTruchetTiling(containerSquare, tilingAreaWidthLength, truchetSettings) {
    ...
    for (let j = 0; j < truchetRows; j++) {
        ...
        for (let i = 0; i < truchetRows; i++) {
            tileMatric[j][i] = new TruchetTile(tileWidth, truchetSettings);
            tileMatric[j][i].blockContainer.style.top = `${(tileWidth * j)}px`;
            tileMatric[j][i].blockContainer.style.left = `${(tileWidth * i)}px`;
