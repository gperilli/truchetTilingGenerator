    ...
    for (let i = 0; i < joinerGroupsToJoinUniqArray.length; i++) {
            ...
            joinedGroups.forEach((joinedGroup) => {
                ...
                for (let j = 0; j < joinerGroupsToJoinUniqArray.length; j++) {
                    ...
                    if (joinerGroupsToJoinUniqArray[j] != joinerGroupsToJoinUniqArray[i]) {
                        joinedGroup.forEach((group) => {
                            if (group == joinerGroupsToJoinUniqArray[j][0] || group == joinerGroupsToJoinUniqArray[j][1]) {
                                group.push(joinerGroupsToJoinUniqArray[j][0]);
                                group.push(joinerGroupsToJoinUniqArray[j][1]);
                                joinerGroupsToJoinUniqArray[j].merged = true;
                            }
                        })
                    }
                }
