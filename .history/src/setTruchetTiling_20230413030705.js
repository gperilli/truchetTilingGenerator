...
function setTruchetTiling(containerSquare, tilingAreaWidthLength, truchetSettings) {
                ...
                contiguousGroups[i].forEach((ipart) => {
                    contiguousGroups[j].forEach((jpart) => {
                        ...
                        if (ipart == jpart) {
                            // console.log("mklmkl");
                            newContiguousGroups[i] = [];
                            newContiguousGroups[i].push(contiguousGroups[i].concat(contiguousGroups[j]))
                        } else {
                            // newContiguousGroups[i] = [];
                            // newContiguousGroups[i].push(contiguousGroups[i])
                        }
                    })
                })
            }
    ...
    contiguousGroups.forEach((group) => {
     ...
     newContiguousGroups.forEach((group) => {
         let randHexColor = "#" + Math.floor(Math.random()*16777215).toString(16);
         group.forEach((part) => {
             if (group.length > 1) {
                console.log(part[0])
                part[0].forEach((conPart) => {
                    if (conPart.className.baseVal == 'truchetBottomLeftToTopRightBack' || conPart.className.baseVal == 'truchetBottomRightToTopLeftBack') {
                    // console.log("bang");
                    conPart.style.background = randHexColor;
                } else {
                    conPart.style.fill = randHexColor;
                }
                })
