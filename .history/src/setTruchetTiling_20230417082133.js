...
function setTruchetTiling(containerSquare, tilingAreaWidthLength, truchetSettings) {
    ...
    let highestLevelGroups = [];
    for (let j = 0; j < groupsToJoinUniqArray.length; j++) {
        for (let i = 0; i < groupsToJoinUniqArray[j].length; i++) {
            
            groupsToJoinUniqArray.forEach((group) => {
                    if (group != groupsToJoinUniqArray[j] && group.includes(groupsToJoinUniqArray[j][i])) {
                        highestLevelGroups.push([group, groupsToJoinUniqArray[j]]);
                    } else {
                        highestLevelGroups.push(groupsToJoinUniqArray[j]);
                    }
            })
        }
    };

    console.log("highestLevelGroups");
    console.log(highestLevelGroups);
