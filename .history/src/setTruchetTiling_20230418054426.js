...
function setTruchetTiling(containerSquare, tilingAreaWidthLength, truchetSettings) {
    ...
    let joinerArray = [];
    //joinerArray.push(joinerGroupsToJoinUniqArray[0].group);
    
    for (let i = 0; i < joinerGroupsToJoinUniqArray.length; i++) {
        
        for (let j = 0; j < joinerArray.length; j++) {
            
            if (joinerArray.length > 0) {
                for (let k = 0; k < joinerArray[j].length; k++) {
                    if ((joinerArray[j][k] == joinerGroupsToJoinUniqArray[i].group[0] || joinerArray[j][k] == joinerGroupsToJoinUniqArray[i].group[1]) && joinerGroupsToJoinUniqArray[i].checked == false) {
                        joinerArray[j].push(joinerGroupsToJoinUniqArray[i].group[0]);
                        joinerArray[j].push(joinerGroupsToJoinUniqArray[i].group[1]);
        ...
        if (joinerGroupsToJoinUniqArray[i].merged == false && joinerGroupsToJoinUniqArray[i].checked == true) {
            joinerArray[joinerArray.length + 1] = joinerGroupsToJoinUniqArray[i].group;
        }

        //joinerArray.push(joinerGroupsToJoinUniqArray[i]);
