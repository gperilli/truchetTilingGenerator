...
function setTruchetTiling(containerSquare, tilingAreaWidthLength, truchetSettings) {
    ...
    for (let i = 0; i < joinerGroupsToJoinUniqArray.length; i++) {
            ...
            for (let l = 0; l < uncheckedGroups.length; l++) {
                ...
                if (uncheckedGroups[l].group != joinerGroupsToJoinUniqArray[i].group) {
                    ...
                    for (let k = 0; k < joinedGroups[m].length; k++) {
                        console.log("bang");
                        if ((joinedGroups[m][k] == uncheckedGroups[l].group[0] || joinedGroups[m][k] == uncheckedGroups[l].group[1]) && uncheckedGroups[l].merged == false) {
                            joinedGroups[m].push(uncheckedGroups[l].group[0]);
                            joinedGroups[m].push(uncheckedGroups[l].group[1]);
        ...
        if (joinerGroupsToJoinUniqArray[i].merged == false) {
            joinedGroups.push(joinerGroupsToJoinUniqArray[i].group);
