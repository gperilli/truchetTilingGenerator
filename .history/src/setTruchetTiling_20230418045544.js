...
function setTruchetTiling(containerSquare, tilingAreaWidthLength, truchetSettings) {
    ...
    console.log("trimmed groups:");
    console.log(joinerGroupsToJoinUniqArray);
    
    let joinedGroups = [];
    // joinedGroups.push(joinerGroupsToJoinUniqArray[0]);
    // joinerGroupsToJoinUniqArray[0].merged = true;

    for (let i = 0; i < joinerGroupsToJoinUniqArray.length; i++) {
 
        //joinedGroups.forEach((joinedGroup) => {
        for (let m = 0; m < joinedGroups.length; m++) {
            
            // compare each joined to unjoined group
            let uncheckedGroups = [];
            for (let j = 0; j < joinerGroupsToJoinUniqArray.length; j++) {
                if (joinerGroupsToJoinUniqArray[j].checked == false) {
                    uncheckedGroups.push(joinerGroupsToJoinUniqArray[j]);
                }
            }

            for (let l = 0; l < uncheckedGroups.length; l++) {

                // don't join the same sub arrays in the same array
                if (uncheckedGroups[l].group != joinerGroupsToJoinUniqArray[i].group) {
                    //console.log(`joined groups length: ${joinedGroups.length}`);
                    //console.log(joinedGroups[0].group)
                    for (let k = 0; k < joinedGroups[m].length; k++) {
                        console.log("bang");
                        if ((joinedGroups[m].group[k] == uncheckedGroups[l].group[0] || joinedGroups[m].group[k] == uncheckedGroups[l].group[1]) && uncheckedGroups[l].merged == false) {
                            joinedGroups[m].group.push(uncheckedGroups[l].group[0]);
                            joinedGroups[m].group.push(uncheckedGroups[l].group[1]);
                            uncheckedGroups[l].merged = true;
                        }
                    }
                }
            }
        ...
        joinerGroupsToJoinUniqArray[i].checked = true;
        if (joinerGroupsToJoinUniqArray[i].merged == false) {
            joinedGroups.push(joinerGroupsToJoinUniqArray[i]);
            joinerGroupsToJoinUniqArray[i].merged = true;
        }
