    ...
    for (let j = 0; j < truchetRows; i++) {
        for (let i = 0; i < truchetRows; i++) {
            ...
            if (i == 0 && j == 0) {
                ...
                console.log(this.topLeftClass);
                if (this.topLeftClass == 'truchetTopLeft') {
                    contiguousGroups[contiguousGroups.length].push(this.topLeftElement);
                }
            } else if (i == (truchetRows - 1) && j == 0) {
                // top right corner tile
                let tileAbove = nil;
                if (this.topRightClass == 'truchetTopRight') {
                    contiguousGroups[contiguousGroups.length].push(this.topRightElement);
                }
            } else if (i == 0 && j == (truchetRows - 1)) {
                // bottom left corner tile
                let tileToTheLeft = nil;
                if (this.bottomLeftClass == 'truchetBottomLeft') {
                    contiguousGroups[contiguousGroups.length].push(this.bottomLeftElement);
                }
            } else if (i == (truchetRows - 1) && j == (truchetRows - 1)) {
                // bottom right corner tile
                if (this.bottomRightClass == 'truchetBottomRight') {
                    contiguousGroups[contiguousGroups.length].push(this.bottomRightElement);
                }
