...
function setTruchetTiling(containerSquare, tilingAreaWidthLength, truchetSettings) {
    ...
    groupsToJoinUniqArray.forEach((group) => {
        joinerGroupsToJoinUniqArray.push({joined: false, group: group});
    });

    console.log(joinerGroupsToJoinUniqArray);
    
    // let joinedGroups = [];
    // joinerGroupsToJoinUniqArray[0].joined = true;
    // for (let i = 0; i < joinerGroupsToJoinUniqArray.length; i++) {
    //     //if (joinerGroupsToJoinUniqArray[i].joined == true) {
    //         // compare each joined to unjoined group
    //         for (let j = 0; j < joinerGroupsToJoinUniqArray.length; j++) {
    //             if (joinerGroupsToJoinUniqArray[j].joined == false) {
    //                 joinerGroupsToJoinUniqArray[i].forEach((joinedGroup) => {
    //                     if(joinedGroup.group == joinerGroupsToJoinUniqArray[i][0].group || joinedGroup.group == joinerGroupsToJoinUniqArray[i][1].group) {
    //                         
    //                     }
    //                 })
    //             }
    ...
    myMergedArr = joinerGroupsToJoinUniqArray.reduce((merged, array) => {
