...
function setTruchetTiling(containerSquare, tilingAreaWidthLength, truchetSettings) {
    ...
    let topLevelContiguousGroups = ["ftfty"];
    for (let y = 0; y < truchetRows; y++) {
        for (let x = 0; x < truchetRows; x++) {
            if (topLevelContiguousGroups.includes(tileMatrix[y][x].colorGroup) == false) {
                topLevelContiguousGroups.push(tileMatrix[y][x].colorGroup);
            }
        }
    }
